...
>
> 最后更新：2026-09-25 ｜ 当前版本 **v2.3.0**（已发布 + OTA）
...
| 测试 | `npm test` = `node --test "test/**/*.test.mjs"` → 当前 **799 用例全绿**（61 个测试文件） |
...
- 已知遗留（非阻塞）：
  ...
  - 移动版（`JSK管理APP`）**未同步**桌面版修复 → 待办已**独立成文**：
    [`docs/规格与计划/测卡工作区/移动版同步-待办清单.md`](docs/规格与计划/测卡工作区/移动版同步-待办清单.md)（M1~M7 逐项取证要点）。

> **后续进展（2026-09-21）**：查重 / 扫描 / 检索全链路修复 **Phase 1+2+3 已全部落地**
> （AR-39 / AR-40 / AR-41 / DF-17 / DF-18 / DF-19 / PK-18 / PK-19；471 用例全绿；**未打包、未推送 Release**）。
> 详见 [`docs/规格与计划/查重引擎/查重扫描与检索-最终方案.md`](docs/规格与计划/查重引擎/查重扫描与检索-最终方案.md) 与
> [`剩余任务.md`](docs/规格与计划/查重引擎/查重扫描与检索-剩余任务.md)（Phase 3 已完成）。
>
...
> 详见 [`docs/规格与计划/世界书大库/世界书大库-加载与查重架构方案.md`](docs/规格与计划/世界书大库/世界书大库-加载与查重架构方案.md)、
> [`S0.5-simhash特征方案实验报告.md`](docs/规格与计划/世界书大库/S0.5-simhash特征方案实验报告.md) 与
> [`后续计划-20260922-PK27架构改造收尾.md`](docs/规格与计划/计划与记录/后续计划-20260922-PK27架构改造收尾.md)。
...
test/              61 个测试文件 / 799 用例（node:test，`npm test`）
...
**完整流程、判定标准、回滚口径见 [`docs/发布/规范与流程/一条龙-发布流程.md`](docs/发布/规范与流程/一条龙-发布流程.md)**（前置自查 → 版本号 → 文档三件套 → 打包 → 产物校验 → 装包自测 → 提交打 tag → Release → OTA 验证 → 覆盖发布 → 回滚）。
...
6. 若用户说「一条龙」：照 `docs/发布/规范与流程/一条龙-发布流程.md` 走，注意内部/外部文档分工。
