...
export function buildTagClassificationSystemPrompt(customCats = []) {
    ...
    L.push('');
    L.push('判定纪律（严格遵守）：');
    L.push('1. 专有名词优先归 other：作品名（动漫/游戏/小说/影视/音乐）、IP 品牌、具体角色/人物名、公司平台名、作者名、版本号、网址等，即使字形接近通用词也一律归 other，绝不强行套分类。');
    L.push('2. 一词只归一类，取最贴合的一个分类；严禁重复列出同一标签。');
