...
[⬇️ 下载安装包 `sillytavern-card-manager-2.3.0.exe`](https://github.com/tian2418671-sys/JSKZX/releases/latest)

**📦 绿色免安装版**（解压即用，无需安装）：

[⬇️ 下载绿色版 `sillytavern-card-manager-2.3.0.zip`](https://github.com/tian2418671-sys/JSKZX/releases/latest)
...
- **v2.3.0** —— ①**世界书大升级**：贴标签（点标签即筛选）+ 标签/分组组合筛选、AI 打标（自动识别当前库，范围可选）、自动分组（收纳条件先预览、后执行、可一键回滚）、分组=物理文件夹（整组改名/解散/一键迁移）、保存格式与酒馆原生完全一致、统计收进「📊」浮层；②**世界书大库提速**：上千本近秒开（以前半分钟以上），大库加载/查重/对比不再崩溃，查重提速到几秒~十几秒，对比窗口词条级对齐（新增绿/缺失红）+ 变更行精确高亮，搜索更快更准；③**AI 打标更好用**：窗口加宽 + 左侧分区导航、提示词链路化（破限 → 系统 → 任务 → 预填充，三套现成文案）、一次请求打包多卡、断了可续跑、可跳过已打标卡、超长卡自动分段、一键测试接口连通性；④**查重更准更稳**：卡内世界书参与比对、预设「改名同源」可查出、重复类型处理建议 + 按重合百分比分档、结果按综合分排序；⑤**长期记忆逐条编辑**；⑥修复：WebP 卡保存自动升级为 PNG（不再静默不落盘）、导入支持格式口径统一、点「全部」分组不再无响应等
- **v2.2.13** —— ①**打标三层开关**：规则 / 本地向量 / AI 兜底逐层独立开关（含 38 条内置规则逐条启停）；②**「打标过程」窗口**：启动打标自动打开应用内实时日志窗（逐卡命中 / 请求 / 成功 / 失败归类 + 管线开关说明），不再弹系统框；③**卡片自动分组**（新）：给分组配收纳条件，未分类卡片「先预览、后执行、可一键回滚」自动归位（推荐模板 + 🤖 AI 分辨兜底，建议默认不勾选）；④**「📁 分组」菜单**：新建 / 重命名 / 删除 / 自动分组 / 批量归类 / 空组清理集中入口；⑤收纳规则**点选式填写**（多词点选或回车添加，不必手写正则；「✨ 生成」一键起草 AI 判定标准）；⑥**侧栏每页显示数量**：角色卡 / 世界书 / 预设 / 插件四库 15~115 共 11 档共用、重启保留；⑦「命令面板」Ctrl+Shift+P + 菜单归位；⑧修复：批量操作条 / 打标过程窗口被遮挡（层级）等
- **v2.2.6** —— ①**预设缝合中心**：把 1~N 本源预设的条目 + 手写自定义条目缝成目标预设（新建 / 覆盖 / 写回当前三种输出），撞名逐条决策（覆盖 / 保留 / 重命名共存 / 跳过）、落位可控（尾部 / 内置位次 / 锚点）、👁 干跑预览不写盘、⭐ 常用条目库；同步**重建 `prompts` 与 `prompt_order`**（酒馆实际按 order 执行，此前本工具不维护它，只搬 prompts 会顺序错乱）；②**四处条目批量操作**：角色卡正则栏 / 角色卡内嵌世界书 / 世界书库词条 / 预设正则区统一支持批量 启用·停用·克隆·删除（世界书另有常驻·条件），切卡切书切预设自动退出；③修复内嵌世界书**字典形态卡**点新增/导入会**清空整本词条**（数据丢失级）与批量删除/克隆**静默失效**；④缝合中心上手引导强化（常驻流程条 + 详细说明 + 悬停提示）
...
- `dist/sillytavern-card-manager-2.3.0.exe` —— NSIS 安装包
...
├── test/                   # node:test 单元测试（742 用例 / 60 个文件）
