# SillyTavern 角色卡管理器 · v1.0 → v2.2.14 更新汇总
...
> **🩹 v2.2.14（2026-09-21）**：自动分组「重新扫描」引导修复（AR-38）——未配置/未保存收纳规则时点击不再是静默无反应，
> 就地提示「先添加规则并保存」；预览区空态文案跟随已保存规则状态。
...
> **🩹 v2.2.13（2026-09-20）**：P1 打标三层漏斗开关 + P2 命令注册表 / 命令面板 / 菜单归位 + 全局资产库下线
...
## 🩹 v2.2.14（2026-09-21）· 自动分组「重新扫描」引导修复（AR-38）
