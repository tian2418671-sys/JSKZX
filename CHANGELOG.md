...
## 🚀 v2.3.0 · 世界书库：标签编辑 + AI 打标统一入口 + 候选池开关 + 物理分组/自动分组（S1~S4，2026-09-25）

> 方案与实施记录：[`docs/规格与计划/世界书大库/世界书-标签与AI打标及自动分组-方案.md`](docs/规格与计划/世界书大库/世界书-标签与AI打标及自动分组-方案.md)（Q1~Q7 已拍板 · 已落地）。

### 🧩 实现要点（内部）

- **S1 世界书标签**：`useWorldbooks.js` 新增 `addWbTagOn` / `removeWbTagOn` 原语；**修复响应式缺口**（`wb.tags` 在 shallowRef 对象上不触发更新、且 `getWbTags` 内存优先会短接依赖收集 → `setWbTags` 出口统一补 `triggerRef(worldbooks)`，一处修全——覆盖手动编辑/批量/打标全部写入路径）；新组件 `WbTagEditModal.vue`（现标签 ✕ / 输入框回车添加 / 全库标签建议点选）；`SidebarPanel.vue` 世界书列表渲染标签 chips（点击=按标签筛选）+ 🏷️ 按钮；`WbContextMenu.vue` 增「🏷️ 编辑标签」「🤖 AI 打标」。
- **世界书库统计 → 折叠浮层**（同日用户要求）：原常驻 3x2 网格统计删除（列表恢复全高），改为行2 「📊」按钮展开的 sb-popover 浮层（与 ▾ 高级工具区**同位置互斥**；点外部即关，复用 onDocPointerDown；`toggleWbStats`/`toggleWbAdvanced` 互斥函数）；6 格（本/词条/Token/常驻/触发/均条）内容原样。
- **S2 候选池增强**：`useCandidatePool`（默认开）+ `enableAIExtraction` + `aiCandidateTags` **三状态上移 App.vue 并持久化**（`ui.candidatePoolEnabled` / `ui.enableAIExtraction` / `ui.aiCandidateTags`；老配置零迁移）；`composeTagPromptHead()`（llmPromptRoles.js 新纯函数——池关：不输出池段落与规则、附加要求仍保留）；`resolveFunnelPlan(..., { poolDisabled })`（池关 → **②向量跳过**并给出原因：向量层的标签源就是池）；**Q1 真值表**可用性判定（`candidatePoolSwitchable` = `(①≡②)&&③`，禁用态保留可见+原因 tooltip）；**产出回池**（`offerPoolRefill`：收尾「加入 / 放弃」整批二选一，池关不询问，批量入池只写一次盘）。
- **S3 世界书打标（与角色卡同一条系统）**：共享设施提升到 setup 层——`buildTagPromptHead()` / `requestTaggingShared(llmOnly,…)` / `callAIWithRetry` / `parseTagReplyShared()`（卡版两处内联解析同步收敛到此）；`startWbTagging()`：范围=当前书 / 当前筛选结果；材料=书名+词条 key/content（**不截断**，超长按段落分段，**单本 40 段上限+均匀采样**防 6MB 级书千段请求）；落盘 `wbTagMap`（**不写文件**）；逐本按需加载 + 用后释放（内存纪律）；`openAITagModal` 按 `appMode` 分发；`AITagModal.vue` 双资产（世界书徽标 / 范围单选 / 打包与断点续跑仅卡片可见）；`HeaderBar.vue` 工具栏「🤖 AI 打标」按视图分流（与关系图谱/智能查重同款）。
- **S4 物理分组（Q6 甲：物理文件夹为唯一标准）**：`wbFolderGroupOf()`（相对路径→物理一级目录推导）；`getWbCategory` 改为**物理优先、虚拟（wbCategoryMap）兼容**；`moveWbToFolder()`（建夹→`wb:rename`→同名冲突自动后缀→**分组+标签双键迁移**→`triggerRef`）；`changeWbCategory`（右键「移动分组」）改物理；`renameWbGroup`（`fs:renameGroupFolder` 整体改名 + 内存 path 前缀替换 + 键迁移）/`deleteWbGroup`（移回库根 + 删空夹）；`migrateVirtualGroupsToFolders()` 迁移助手（存量虚拟分组一次性物理化后清映射；支持 `skipConfirm` 供 e2e，与 `useAutoGroup` quiet 同口径）；新增 `utils/wbAutoGroup.js`（判定纯函数：关键词/正则 + LLM 判定层消息与解析）+ `useWbAutoGroup.js`（材料=书名+词条名（零正文读取）；执行/回滚按**书名**定位不记绝对路径）+ `WbAutoGroupModal.vue`（双选项卡对齐 AutoGroupModal：📋 收纳规则+迁移助手 / 🔍 预览与执行+AI 判定）；命令 `tag.autoGroup` 改按视图分发（`openAutoGroupEntry`）。
- **持久化**：新增 `ui.wbAutoGroupProfiles` / `ui.wbAutoGroupLastRun`（恢复段兼容老配置）；`migrateWbPathKeys` 统一「分组+标签」双键迁移（顺带修 A2 遗留缺口：此前改名世界书只迁分组键、标签键会丢）。

### 🧪 验证

- `npm test` **799 全绿**（+38：`wbAutoGroup` 判定 22 条 + `tagFunnel` 池关 4 条 + `llmPromptRoles` 头部组装/分段 12 条）；`guard:scope` 0 未定义；`ctx_exposure` / `command_registry` / `auto_group_safety` 三 pychecks 全过；`npm run build:web` ✅；真实启动冒烟无 `[Vue 错误]`（**首冒烟即抓获**：模板引用的 6 个新函数未进 ctx；随后补齐 `addWbTagOn/removeWbTagOn/getWbCategory/wbCategoryMap/releaseWorldbookBody/aiTagTargetMode` 等 ctx 导出）。
- **真实库只读探针** `scripts/probes/_probe-s4-wb-read.mjs`（H:\01\全局世界书，39 个 json → 载入 35 本）**14/14**：物理子文件夹「XP三册」被正确推导为分组名；贴标签 → 列表 chips 即时渲染；右键菜单/编辑面板（增删）/打标弹窗（世界书模式+范围+池开关）/自动分组弹窗（双选项卡+迁移助手）全通过；9.9MB 大书（女神攻略调教手册）懒加载 1905 词条 + 用后释放（`dataLoaded=false`）。
- **副本写链路探针** `scripts/probes/_probe-s4-wb-write.mjs`（隔离副本库）**23/23**：物理移动后**标签键迁移**（旧键无 / 新键有——铁律 6）；分组重命名全链路（含 prompt 弹窗）；自动分组扫描→执行→回滚（全部解决后日志清空）；迁移助手（文件进同名文件夹 + 映射表清空）；**打标全程 MD5 0 字节改动**（「不写世界书文件」的硬校验）；零渲染错误。

---

## 🚀 v2.3.0 · AI 打标第二批：提示词链路化 + 批量提量（打包/续跑）+ 超长卡分段（2026-09-25）

> 方案与实施记录：[`docs/规格与计划/AI打标/AI打标-提示词改造与批量提量方案.md`](docs/规格与计划/AI打标/AI打标-提示词改造与批量提量方案.md)（已落地 · §十）。

### 🧩 实现要点（内部）

- `js/utils/llmPromptRoles.js`：单套消息组装 `{system,user,prefill}`（破限拼 system 末尾、预填为末尾 assistant）；`migrateLegacyPresets()`；`SYSTEM_PROMPT_VARIANTS`（standard/deep/brief）+ `resolveSystemVariantId()`；新增 `packedOutputRule()` / `parsePackedTags()`（多卡 `<tags>{"1":[…]}</tags>`，对象键防错位；取最后一个有效包裹防思维链示例污染）；移除 CoT 全家（COT_MODE_* / resolveCotPrompt / willUseCot / DEFAULT_COT_PROMPT / normalizePromptPreset(s) / hasRoleFields）。
- `js/composables/useAITools.js`：`requestTagging()` 统一请求出口（两级降级：全量 → 去预填充）；打包（短卡 token 估算 ≤1200；多卡输出逐卡校验；缺失/整包失败自动拆单发；1.5s 按请求计）；分段（阈值 4000 字 / 单段 ≤3500 字按段落切；合并去重按出现次数排序）；卡片材料**不再硬截断**（原 1500/500/300 取消）；候选池上限 300；断点续跑账本（成功才记、空结果记完成、卡缺失提示跳过、deep-watch 实时防抖落盘）。
- `js/components/AITagModal.vue`：预设库/五小页签/独立破限区 → 链路化 UI（System 含破限+预设套用 / User+打包滑块 / 预填充折叠）；导航 7 → 6 项；执行管线页增打包滑块 + 续跑入口。
- `js/components/App.vue` + `js/composables/useConfigPersistence.js`：新增持久化 `ui.llmRolePrompts` / `ui.tagPackSize` / `ui.tagResume` / `ui.tagSkipTagged`；旧 `ui.systemPromptPresets` 保留在配置文件（可回滚）；`__jskDiag.aiTag.rolePrompts()/setRolePrompts()` 探针口更新（含 `skipTagged`）。
- `js/utils/tagIncrement.js`（新，增量模式）：`hasAnyTag()`（`customTags` / `data.data.tags` / `data.tags` 任一非空）、`splitByTagged()`；`useAITools.js` 在目标构建后过滤已打标卡（被跳过卡在续跑账本记完成防死循环；全跳过 → 清账本 + 提示）；`AITagModal.vue` 执行管线页加复选开关。新单测 `test/tagIncrement.test.mjs`（10 条）。
- **修复（AI-10 · 同日打标首测发现并根治）**：`parseStructuredTags(rawReply, { prefill })` 支持「**拼回预填充再解析**」（`requestTagging` 返回 `usedPrefill`，随降级阶梯传递）——发送侧 assistant 预填充（`<tags>[`）与回复（续写）在解析侧对齐，①②正常命中、不再全落③；**打包请求禁用预填充**（对象 `{…}` 与数组 `[` 本就冲突，从根上不再打架）；新增 `sanitizeTagList()` 全频谱兜底清洗（层①-b/③ + 两处非 llm-only 兜底共用；详见 `docs/bugs/BUG-AI打标与标签.md` AI-10）。

### 🧪 验证

- `npm test` **761 全绿**（删 12 条 CoT 用例；增 11 条迁移/变体/打包解析 + 10 条增量模式 + 5 条预填充拼接 + 14 条兜底清理）；`guard:scope` 0 未定义；`build:web` ✅；真实启动冒烟无 `[Vue 错误]`。
- 新增 CDP 探针 `scripts/probes/_probe-aitag-chain-v2.mjs`（**22/22**，含增量开关 ⑦c 可勾选 / ⑦d 诊断读数 `skipTagged`）与 `_probe-app-boot-debug.mjs`（启动诊断）；`_probe-aitag-nav.mjs` 同步为 6 分区；删除 `_probe-aitag-cot.mjs`。
- **真实小库热测（2026-09-25）**：`_probe-aitag-hot.mjs` 适配 6 分区 + `__probeGetCtx` 后 **29/29**（分区互斥 / 破限联动 / 开合循环 / 窄窗响应式 / 规则表 / 零渲染错误）；`_probe-aitag-chain-v2.mjs` **22/22**（真实库直接开弹窗，不再注入探针卡）。
- **增量模式端到端（副本库 + mock API，写操作隔离）**：新探针 `scripts/probes/_probe-aitag-increment.mjs` **19/19** —— 混合（2 已打标 + 1 未打标）仅发 1 个请求且请求体只含未打标卡、未打标卡落标签、已打标卡零触碰（内存 + PNG mtime 双证）；全跳过 → 0 请求 / 0 日志 / 进度保持。
- **根治验证（AI-10）**：副本库 + 续写形态 mock（`热测标签A、热测标签B、</tags>`，模拟预填充续写）**10/10** —— 单卡链标签干净落盘且日志**不含「走第③层兜底拆分」**（根治生效）；打包请求 `roles=[system,user]`（**无 assistant 段**，预填充已禁）。
- 记录：prod 构建下 `app._instance` 为 null、`__jskDiag` 不存在（探针必须在 dev 模式跑）—— 见 `_probe-aitag-chain-v2.mjs` 头注释。

---

## 🚀 v2.3.0 · 查重引擎 v4 全量落地（P0~P2，2026-09-25）

> 承接同日「旧实现整体下线」，v4 按「三轨判定 → 星型聚类 → 视图 → 清理」从零重建。
> 规格（已通过评审·全票 Approved）：[`docs/规格与计划/查重引擎/查重引擎重构方案-v4-实现规格（通用算法版）.md`](docs/规格与计划/查重引擎/查重引擎重构方案-v4-实现规格（通用算法版）.md)（实施记录 §15.1 / §15.2）
> 资料汇编：[`docs/规格与计划/查重引擎/查重-全量资料汇编.md`](docs/规格与计划/查重引擎/查重-全量资料汇编.md)

### ✨ 用户可感知的变化

- **四个查重入口恢复可用**（角色卡 / 世界书 / 预设 / 内容级）；差异窗口**三分支**（世界书词条级 / 预设触发词与标签 / 卡片行级着色）自动识别，无需手选。
- **真实库零假阳性**：世界书 39 本检出 2 组（L1 证据 100%）、卡库 77 张检出 1 组（L0）；67 / 46 对候选中 61 / 45 对被证据门槛正确拒绝。
- **清理更稳**：白名单内入回收站 + 内存同步；白名单外**显式失败**（DF-27：不再把「未处理」当「已删」）。
- 差异窗口**正文未载入时明确提示**，不再静默当空。

### 🧩 实现要点（内部）

- P1：8 个纯函数模块（`simhash64.mjs` + `dedupeCommon / Names / Contract / Buckets / Gates / Cluster / View`）+ `useDedupe.js` 编排重写（三轨 → 判定 → 星型聚类 → 视图 → 清理）+ 静态守卫 `guard:dedupe`（API 红线 + 零案例字面量，注释同样扫描）。
- P2：`dedupeDiff`（三分支 + 异步正文读回）/ `dedupeDf`（模板剥离全量：行级片段 ≥60 字 / 跨卡 ≥5 判模板 / 克隆保护；`runCardPipeline` 两遍 Pass A→B）/ `dedupePreset`（三维五枚举；`prompt_order` 取覆盖最多组——DF-24）。
- 校准抓获并修复 4 个实现缺陷（均已入回归）：MinHash 采样错位（排序等距 → 确定性哈希）、版本形状边界（`2.2版` lookbehind）、视图簇心置首、DF-27 幽灵移除。
- **性能复核修复「正文锚定」**：Pass B 包装对象进 `raw` 会阻止 `slimCard` 交还正文 ⇒ 全库正文被结果集锚住（rss 1,724MB → **325MB**，↓81%）。

### 🧪 验证

- `npm test` **733 全绿**（P1 +67 / P2 +23 用例）；`guard:scope` 0 未定义；`guard:dedupe` 13 文件零违规；`npm run build:web` ✅；真实启动冒烟（四入口 + 差异窗口 + 隔离库清理双用例）全通过。
- 性能基线（真实库）：11k 卡全流程 **~82s**（目标 ≤180s）/ 世界书千本 **1.8s**（合成规模压测，目标 ≤30s）/ rss **325MB**、heapUsed 121MB。
- 11k 管线：候选 291,137 / 证据边 9,060 / 重复组 1,907（L0=1759 / L1=124 / L2=24）/ 同名家族 244。

---

## 🗑️ v2.3.0 · 查重旧实现**整体下线**（推倒重来，2026-09-25）

> 用户决定重建：**旧查重代码全部删除**，只保留「**查重 UI**」与「**三合一切换**」。
> 备份（全文 + 单测 + 探针 + 未提交 diff）：`%USERPROFILE%\jsk-dedupe-backup-20260925\`

### 🗑️ 已删除（决策逻辑全清）

- `js/composables/useDedupe.js`（约 3000 行）→ 重写为**最小壳**：只留弹窗/进度/差异窗口的空状态 ref、
  四个入口占位（点击提示「已下线」）与 `startSmartDedupe()` **三合一分发**（角色卡 / 世界书 / 预设）。
- 查重专用工具：`js/utils/similarityType.js`、`js/utils/presetStructure.js`、`js/utils/dedupeBusy.js`。
- App.vue 查重接线：依赖注入袋（30 项 → 4 项）、`isDedupeBusy` 与「查重期延后瘦身 + 补跑 watch」。
- 查重单测 10 份（AR-50 模板/否决、卡字段口径、相似类型、进度状态机、L1 快照、simhash、预设结构等）。
- 查重专用探针 49 份 + `scripts/tools/make-preset-lib.mjs`；守卫白名单同步移除 useDedupe.js。

### ✅ 保留（点名保留 + 共用基础设施）

- **UI**：`DedupeModal` / `WbDedupeModal` / `PresetDedupeModal` / `ContentDedupeModal` /
  `DedupeScanProgress` / `DiffModal` + 模板绑定与入口按钮/命令（`edit.dedupeSmart` / `edit.dedupeContent`）。
- **三合一入口**：`startSmartDedupe()` 按当前库类型分发（新实现直接替换占位函数体）。
- **共用基础设施**（属扫描/加载链路，非查重决策）：L1 索引（`keyHashes` / `exactContentHash` / `simhash`）、
  `consumeWorldbookBodies` / `releaseWorldbookBody`、`cardSlim`、`library:scan-progress` 重扫进度通道、
  `textDiff` / `entryAlign`（供保留的 DiffModal 渲染）。

### 🧪 下线后验证

- `npm test` **643/643 通过**（原 788，差额 = 删除的查重单测）｜ `guard:batch-read` / `guard:scope` 通过。
- `npm run build:web` 通过；真实卡库（76 张）启动冒烟：入口调用无异常、四个弹窗可渲染、无控制台错误。

---

## 🩹 v2.3.0 · 内容查重「误报四形态」根治 + 提醒按百分比分档（2026-09-25）

> 缺陷登记：[AR-50](docs/bugs/BUG-架构与渲染.md)（误报四形态 + 口径漂移）、[AR-51](docs/bugs/BUG-架构与渲染.md)（提醒与百分比自相矛盾）
> 改动文件：`js/composables/useDedupe.js`、`js/utils/similarityType.js`、`js/components/ContentDedupeModal.vue`、
> `js/components/PresetDedupeModal.vue`、`js/components/App.vue`、`js/utils/dedupeBusy.js`（新增）、
> `main.js` / `preload.js`（重扫进度通道，AR-49）
> 新增单测：`test/similarityType.test.mjs`（+7 分档用例）、`test/scopeUndefined.test.mjs`（**新增静态门禁**）、`test/ar50TemplateAndVeto.test.mjs`、`test/dedupeBusy.test.mjs`

### ✨ 用户可感知的变化

- **查重卡片上的「💡 建议」改为按上面的百分比来写**（**每 10 个百分点一档**，0~10% / 10~20% / … / 90~100%，
  **100% 单独一档**）：低档明确「不建议清理」，100% 才说「可以安全清理」。
  以前这行建议来自**另一套判定**，数据不足时会退回「内容基本无关」，而百分比照常显示 100%
  ⇒ 出现「都 100% 了还说无关」。现在**数字与建议同一口径**，不可能再打架。
- **内容查重不再把无关卡聚成一大组**（四种形态各配一条判据）：
  ① 共享同一段通用开场白的两张无关卡；② 一侧根本没有内嵌世界书的卡；
  ③ 搬运站写死的「分享页 + 版权声明」全库模板；④ 不同角色各自动了同一本设定书。
  实测最大的误聚组 **113 张 → 2 张**、全库最大组 **109 张 → 8 张**，顶级 12 组人工复核全为真版本家族。
- **同一次查重不再「换一把尺子」**：以前大库为省内存把卡内世界书正文换出去的时机不确定，
  同一批卡两次查重可能给出不同结果；现在查重开始**先把正文按需读回**、结束**原样交还**。

### 🧩 实现要点（内部）

- **判据（`useDedupe.js` 角色卡分支，按顺序即保护机制）**：
  ① 双口径 OR 入场（全字段 **或** 5 字段 ≥ 0.85）；
  ② **触发词矛盾否决**（两侧各 ≥2 键 且 keys-Jaccard < **0.05**）；
  ④ **不同角色·共用世界书否决**（keys ≥ **0.5** 且 5 字段 < **0.15** 且**卡名相似度 < 0.3**）；
  ③ **全字段分歧否决**（仅一侧 0 词条时，全字段 ÷ 5 字段 < **0.4**）。
  已否决的样本要 `return`，不能落到最后的 `if (bothKeys) return true`（**判据顺序 = 保护机制**）。
- **模板剥离**：两遍扫描，先按字段值统计**文档频率**（`TEMPLATE_MIN_CARDS=5` / `TEMPLATE_MIN_LEN=60`），
  命中的整段在算指纹**前**剔除；剥完无可比对文本的卡**退出比对并如实上报**。
  实测命中 **104 段模板 / 2382 张卡**（其中 **104 张**卡内容是纯模板）。
- **口径一致性（两步）**：`js/utils/dedupeBusy.js`（**模块级普通变量，刻意不用 ref** ——
  `slimLibraryIfNeeded` 在 App.vue setup 前段、`useDedupe()` 解构在后段，跨段引 ref 会踩 TDZ）
  四条查重入口置忙、唯一收尾点 `finishDedupeScan` 清忙、`watch(dedupeScanning)` 补跑；
  **仅此不够** —— 重扫是**增量刷新**（按 path+mtime 复用内存旧对象），已瘦身的卡不会因重扫恢复正文
  ⇒ 必须 `ensureCardsFull` 按需读回（实测 **2.2ms/张**，9557 张 ≈ **21 秒**），用完只重新瘦身**我们读回的那批**。
- **AR-51 分档提醒**：`similarityType.js` 新增 `CONTENT_PCT_ADVICE` + `adviceForContentPct()`；
  `v._pctAdvice = adviceForContentPct(v._simPct)`（世界书/角色卡）、
  预设侧用 `adviceForContentPct(正文重合 ?? 结构重合)`；类型文案降为**徽标 tooltip**（`_simAdvice`）；
  角色卡的 `contentSim` 在无 `_mhSig` 时**退化用展示口径的同一份指纹**（`fullSigOf`）。
  ⚠️ 无数据判定必须**先于**取数（`Number('') === 0` 会把「无数据」伪装成「0%」⇒ 套用最低档「基本无关」）。

### 🧪 验证

- `npm test` = **788 用例 / 0 失败**（原 774 + 分档 7 + 静态门禁 7）；`npm run build:web` ✅
- 🛑 **热测当场抓到一个「单测/构建都看不见」的错**：取签名助手写在了**第一个回调内部**、被**兄弟回调**引用
  ⇒ `ReferenceError: fullSigOf is not defined` ⇒ 查重结果 **0 组**。修好同时**新增静态门禁**：
  `scripts/check-undefined-scope.mjs`（acorn 作用域感知的未定义标识符检查）+ `npm run guard:scope`（并入 `npm run check`），
  单测含**正向对照**（先证明它抓得住事故原形，再断言真实代码 0 命中）。
- `test/ar50TemplateAndVeto.test.mjs` 10/10（含「阈值与生产源码一致」的源码级守卫）；
  `test/dedupeBusy.test.mjs`（忙标志行为 + 四条入口/收尾/watch 位置的源码守卫）；
  `test/similarityType.test.mjs` **32/32**（含「**100% 不得含『无关』**」与「💡 必须读 `_pctAdvice`」的接线守卫）
- 热测试（真实 11k 库 / `--user-data-dir` 隔离 profile）：**1783 组 / 2219 项逐项校验** —— 分档与百分比**一致（违规 0）**、
  `100%` 的 **1849 项**均含「完全一致」且不含「无关」、`≥90%` 的 **2011 项**不含「无关」、11 个档位**每档都有真样本**（无空档）

---

## 🩹 v2.3.0 · 记忆库逐条编辑 + 查重综合分排序 + 世界书规格状态同步（2026-09-24）

> 本轮补完三处「**有 API 无 UI / 算了不用 / 文档没同步**」的收尾缺口（用户点名要求）。
> 新增单测：`test/similarityType.test.mjs` **+6 条**（排序）；新增探针：`scripts/probes/_probe-mem-dedupe-ui.mjs`（**16 条断言**）

### ✨ 用户可感知的变化

- **长期记忆可以逐条编辑了**：以前记忆列表只能**看和删**，写错一个字只能删掉重记；
  现在每条右侧有 **✏ 编辑**，可就地改内容（备忘类还能改「键」）。
  - 编辑时**其他条目的按钮会置灰**（防同时改两条导致状态错乱）
  - **取消**即丢弃草稿，不会把半成品写进去；**Ctrl+Enter** 快速保存、**Esc** 取消
  - 内容清空时保存按钮自动禁用（记忆不能为空）
- **查重结果按「综合分」排序了**：以前同一组里的版本**只按内容长度排**，
  最相似的未必排最前；现在按**综合分从高到低**（最像的排最前），并在卡片上显示 `📊 综合分`。
  > 综合分 = 长度惩罚 ×（0.75×内容重合 + 0.25×触发词重合）。⚠️ **权重未标定**
  > （真实库仅 8 对真重复样本，调权重会过拟合）⇒ **只用于排序展示，不参与「是否同组」的判定**。
- **修复：记忆列表「只看本卡 / 全部」切换后列表不刷新** —— 已修复。
  以前点这两个按钮**只有按钮高亮会变，列表内容纹丝不动**，用户会以为「全部里也没有别的卡」；
  现在切换会真的重新拉取列表。

### 🧩 实现要点（内部）

- **记忆编辑**（`js/components/ChatTestSidebar.vue`）：`updateMemory` 早已就绪（`main/memoryStore.js`
  支持 content/key/cardName/cardPath/type），**缺的只是 UI**。设计：一次只编辑一条（`editingMemId`）
  - 草稿在本地（`memDraft`，取消即丢弃）+ **只提交真正变化的字段**（少写一次盘）
  - **只有 fact 可改 key**（message/summary 的 key 是程序生成的分类键，改了会破坏去重合并语义）
- **查重排序**：把排序抽成纯函数 **`sortByCompositeScore()`**（`js/utils/similarityType.js`）——
  三级回退（`_score` → `_simPct` → `textLen`）、`_score === null` 排最后、**返回新数组不改入参**（可单测）。
  ⚠️ **排序不能动 `master`**：类型判定是逐项 vs `list[0]`（内容最长者），排走它基准版就变了
  ⇒ 做法是**先摘出 master、只排其余项、再拼回开头**。
  🔑 修复的真缺口：`_score` 原本**算了却从未被消费**（不排序、UI 不显示）= 死代码。
- **修复的静默失效**：`memViewMode` 无 `watch` ⇒ 开关只改标志不重拉列表（与 AI-05「开关时灵时不灵」同型）。
- **世界书规格文档状态同步**（纯文档）：`RFC-20260921-WB-TAGS-02` 实际**已落地**（`wbGroupsTags.js` +
  `useWorldbooks` 16 处 API + DF-26），但规格文档还写「待拍板后实施」⇒ 补 §九 落地记录
  （三个决策的最终选择 + 与原方案的偏差 + **仍未做的两项**：批量贴标签 UI / 词条正文入索引）。
- **世界书查重方案「第 2 步」状态同步**：`keys` 已接入打分 + 综合分排序已落地，但**刻意不做权重标定**。

### 🧪 验证

- `npm test` = **757 用例 / 0 失败**（原 751 + 新增 6）
- `npm run build:web` ✅ ｜ 生产版真实启动冒烟 ✅（无 Vue 错误）
- 新探针 `_probe-mem-dedupe-ui.mjs` **16/16**；负向对照（删掉 `watch(memViewMode)`）→ **9/16**（真抓得住）

---

## 🩹 v2.3.0 · AI 打标「提示词分角色 + 结构化输出」（2026-09-24）

> 依据：[`docs/规格与计划/AI打标/AI打标-提示词分角色与结构化输出方案.md`](docs/规格与计划/AI打标/AI打标-提示词分角色与结构化输出方案.md)（R1 + R2，状态已改「✅ 已落地」）
> 缺陷登记：[AI-09](docs/bugs/BUG-AI打标与标签.md)（贪婪正则 → 思考型模型标签被思维链污染）
> 新增模块：`js/utils/llmPromptRoles.js`（纯函数）｜ 新增单测：`test/llmPromptRoles.test.mjs`（**70 条**）
> 改动文件：`js/composables/useAITools.js`、`js/components/AITagModal.vue`、`js/components/App.vue`

### ✨ 用户可感知的变化

- **提示词预设可分段填写**：以前一个预设只有**一个输入框**（所有指令混在一起）；
  现在展开后是 **🧠 System / 💬 Assistant / 👤 User / ⚡ 预填充 / 🔗 思维链** 五个小页签：
  - **System** —— 角色与任务规则（主提示词；破限词仍自动追加在最末尾）
  - **Assistant** —— 示例推理 / 期望输出格式示范（few-shot，引导模型模仿）
  - **User** —— 本次任务指令（**留空**则用程序自动生成的「卡片内容 + 输出要求」）
  - **预填充** —— 强制模型从指定开头续写（默认 `<tags>[`），**大幅提高格式遵守率**
  - **🔗 思维链** —— 让模型先在心里推理再产出标签；**内置默认版开箱即用**，可切「✏️ 自定义」或「⚪ 关闭」
- **打标结果不再被「思维链」搅乱**：思考型模型（GLM / DeepSeek 等）会先输出一大段推理，
  里面的方括号（编号 `[1]`、示例 `["格式"]`）以前会被误当成标签；
  现在要求模型把结果包在 `<tags>[...]</tags>` 里，**从最后一个有效包裹精确截取**，乱码标签基本消失
- **🔌 新增「测试连通性」按钮**（API 引擎设置区）：一条最小请求立刻验证
  Endpoint / Key / Model 是否真的通，并回显耗时与当前模型名 —— 不用等跑了几十张卡才发现连不上
- **模型拒收时不再整卡失败**：预填充 / 思维链被中转站拒绝时**自动逐级降级重试**（日志会写明降到了哪档），
  跑完还会提示「本次实际使用哪档模式」
- **窗口里能看出「这次是不是启用了新结构」**：执行管线区会显示 🟢 徽标 + 🔗 思维链档位，并给出「去分段编辑 →」直达按钮

### 🧩 实现要点（内部）

- **启用条件（用户指定）**：`isLlmOnlyPlan(plan)` = **①规则关 且 ②向量关 且 ③LLM 开** ——
  **只有 LLM 层单独启动**时才启用「分角色结构 + `<tags>` 结构化截取」；
  其他组合（如规则+LLM 同开）**保持原有链路不变**，零回归风险
- **消息顺序**：`system`(主+破限) → `user` → `assistant`(示例) → `assistant`(🧠 思维链) → `assistant`(⚡ 预填充)
  - ⚠️ **所有 `assistant` 必须排在 `user` 之后** —— **Anthropic 协议硬要求**（第一条非 system 消息必须是 `user`，
    放前面会报错；`main.js` 挑出 `system` 后原样透传 messages）
  - ⚠️ **预填充必须在最后**（紧邻模型输出位置，约束力最强）
- **解析三层降级 + 剥思考块**（`parseStructuredTags`，任一层成功即可）：

  | 层 | 手段 | 要点 |
  | --- | --- | --- |
  | 🧹 | 先剥思考块（`<thinking>` / `thinking` / `<reasoning>` / `【思考】` …） | 🛡️ **剥完失败则回退原文**（防误剥真结果） |
  | ① | `<tags>…</tags>` / `<标签>…</标签>` 精确边界 | ⚠️ **取最后一个有效包裹** —— 思考型模型常在思维链里先给「示例 `<tags>["示例"]</tags>`」，取第一个会**把示例当结果** |
  | ② | JSON 数组正则（**从后往前**找第一个能解析成数组的方括号组 → 贪婪兜底处理嵌套） | 思维链里的 `[1]` / `["示例"]` 自动跳过；嵌套数组 `flat(Infinity)` 展平 |
  | ③ | 按 `[,，、\n]` 暴力拆分 | 最后兜底，命中时打 `⚠️` 日志告知「模型未遵守输出格式」 |

- **🧠 思维链三档**（`preset.cotMode`）：`'default'`（**默认**，内置 `DEFAULT_COT_PROMPT`，只读展示）/ `'custom'`（用 `preset.cot`；**留空回退默认版**，不静默失效）/ `'off'`（不注入）。
  内置默认版融合「思维链引导 + 破限」，并**明确要求「推理过程不要输出」**（从源头减少污染）
- **🛡️ 降级阶梯**（原「只降预填充一次」升级为三级）：① 全量 → ② 去预填充 → ③ 去预填充+去思维链；
  ⚠️ 只保留**实际会用到**的档位；降级时日志写明原因与降级目标，跑完提示「本次实际使用哪档」
- **🔌 `testApiConnection`**：最小请求（`content:'hi'` + `max_tokens:8` + `temperature:0`）验证三要素，
  走 `sendChatMessage` 通道（绕 CORS）；不写配置、不改状态；支持 `{silent:true}` 供将来批量前静默预检复用
- **向后兼容**：旧预设只有 `content` ⇒ `normalizePromptPreset` 自动迁移到 `system`（`content` 保留不删）；
  `useConfigPersistence.js` 对 `systemPromptPresets` 整体序列化 ⇒ **新字段自动落盘，无需改动**；
  `main.js` 的 `chat:send` 原样透传 `messages` ⇒ **`assistant` 消息天然支持，无需改动**
- **UI 编辑双写**：`setPresetField` 写新字段时同步镜像旧 `content`（`system` 段），旧逻辑仍能读到

### 🧪 验证

- `npm test` = **751 用例 / 0 失败**（原 681 + 新增 70）
- `npm run build:web` ✅ ｜ 真实启动冒烟 ✅（无 Vue 运行时错误）

---

## 🩹 v2.3.0 · 角色卡与预设查重加固（2026-09-24）

> 依据：[`docs/规格与计划/查重引擎/角色卡与预设查重-方案评估.md`](docs/规格与计划/查重引擎/角色卡与预设查重-方案评估.md) §三 P0
> 缺陷登记：[DF-23](docs/bugs/BUG-数据与文件.md)（角色卡忽略 71.7% 文本）/ [DF-24](docs/bugs/BUG-数据与文件.md)（预设只按名聚类）
> 新增模块：`js/utils/presetStructure.js`（纯函数）｜ 新增单测：`test/presetStructure.test.mjs`（27 条）+ `test/cardDedupeFields.test.mjs`（15 条）

### ✨ 用户可感知的变化

- **预设查重现在能发现「改名同源」**：以前只比对**同名的**预设，
  两个结构几乎一模一样、只是改了名字的预设**永远不会被列出来**；
  现在按「提示词块结构」比对，改名的改版也能查出来（结果里会标「🔔 改名同源」）
- **角色卡查重不再漏掉「长在世界书里的设定」**：以前只比对卡的描述 / 人格 / 场景 / 开场白，
  **卡内世界书（80% 的卡都有）完全不参与** ⇒ 「两版设定几乎相同、只是正文写在世界书里」的卡查不出来；
  现在把卡内世界书、备用开场白、作者备注一并纳入
- **预设查重结果按类型给出处理建议**：区分「完全重复」（可安全清理）/「结构相同换皮」/「内容相同重排」/
  「参数变体」（属有意调参）/「同名但无关」（**勿清理**），高风险类型的清理按钮**降级为红色警示**
- **修复预设查重「内容完全相同」判定恒真**：以前任何两个预设都可能被判成「参数内容完全一致」，
  现在按真实块正文比对（根因是真实预设的提示词是**数组**，旧代码按对象处理导致每块都退化成 `[object Object]`）

### 🧩 实现要点（内部）

- **角色卡文本提取**（`useDedupe.js` `extractContentText`）：5 字段 → 纳入 `character_book` 词条
  （走 `extractBookEntries` 全形态提取器，`keys` + `content`，**与独立世界书分支同口径**）
  以及 `alternate_greetings` / `creator_notes` / `system_prompt` / `post_history_instructions`
- 🛑 **两处必要配套**（不加会引入新缺陷，实测踩到）：
  - **AR-48 阈值作废 → 误报**：文本量级 ×26 ⇒ simhash 距离整体压缩（无关对 **30 → 20~23**，
    落进 `NAME_ONLY_MAX_DIST=24`，而真实 4-gram Jaccard 仅 **1.6%**）
    ⇒ 「同名假阳性防护」**刻意保留 5 字段口径**（新增 `extractLegacyCardText`），保住 AR-48 的 T=24
  - **稀释 → 漏报**：`鬼.png` ↔ `鬼1.png`（真实同源改版）5 字段 100% → 全字段 **67.7%**（< 0.85 闸门）
    ⇒ 聚类闸门改**双口径 OR**（`全字段 ≥0.85 或 5 字段 ≥0.85`），同时覆盖「仅世界书不同」与「正文版本迭代」
- **预设结构指纹**（新模块 `js/utils/presetStructure.js`）：
  `identifier` 集合 Jaccard（结构）+ 按块长度加权的逐块 4-gram Jaccard（内容）
  加 `prompt_order` 的 Kendall tau（顺序）加**启用状态一致性**加采样参数一致性（**五维**）；
  聚类改**结构指纹 + 簇心校验**（`unionFindGated`，**不得**用连通分量 —— PK-29 的病根），
  同名的、结构不相似的**仍列出**但标「同名但无关」（避免功能回退）
- **`prompts` 两种真实形态都兼容**（数组 / 数字键对象）——
  修复「数组被当对象 → `String(块对象)` = `[object Object]`」导致的**内容文本全垃圾** +
  **指纹恒真** + **差异比对器提示词正文显示垃圾**（三处同源缺陷）
- **`pickOrderGroup` 取覆盖度最高的 order 组**，**不写死 `character_id`**（真实库 `100000` / `100001` 并存）
- **新增「🎛️ 启用状态不同」类型**：块开关不同 ⇒ 生效的提示词不同 ⇒ **行为不同**，不得判「完全重复」
- 🎚️ **预设查重阈值转「保守档」**（**宁可漏报，绝不误报**）——
  弹窗按钮是「保留此版，清理其余」，**误报 = 一键误删真数据**；漏报只是「没查出来」：
  ...
  聚类阈值**收敛为单一来源**（`PRESET_CLUSTER_THRESHOLD`）—— 旧实现两处各存一份会漂移。
  ⚠️ 仍**未标定**（真实库仅 5 个 OpenAI 预设 / 1 对同源，不足以标定 3 权重 + 4 阈值）；
  待积累到 20+ OpenAI 预设 / 10+ 对同源后再回头标定，届时**只需改常量**
- **🛑 差异比对器字段与查重字段对齐**（防「查重判重复、比对说一致」⇒ 一键误删）：
  新增 `creator_notes` / `system_prompt` / `post_history_instructions` 三行 +
  「🎁 备用开场白」行 + 「📚 内嵌世界书」**按词条逐条比对**（复用 `alignEntryLists`，与世界书侧同一套「新增/缺失/改动」语义）
- **阈值状态**：`PRESET_STRUCT_THRESHOLD` 等**均为方案经验值、未标定**
  （真实库仅 5 个 OpenAI 预设 / 1 对同源，不足以标定 3 权重 + 4 阈值 —— 必然过拟合，同 PK-29 的教训）；
  代码与文档均标注「启发式、未标定」
- **验证**：`npm test` **652 用例全绿**（52 文件）；`npm run guard:batch-read` 通过；
  `npm run build:web` 通过；真实启动冒烟无 `[Vue 错误]` / 无崩溃；
  真实卡实测（4 张）双口径 OR 闸门 vs 真值 **6/6 一致**

### 🎚️ 阈值标定（合成样本库，2026-09-24）

- 新增 `scripts/tools/make-preset-lib.mjs`（**合成预设库生成器**：造带真值的样本族，
  覆盖 exact / renamed / reskin×4 / trimmed / extended / reorder / flipped / sampler /
  unrelated / sameNameOnly / sharedSkeleton）+ `scripts/probes/_probe-preset-threshold-calib.mjs`（**阈值网格扫描**）
- 实测：**结构阈值 0.95 → 零误报零漏报**（TP=11 / FN=0 / FP=0 / TN=3），
  无关对最高结构相似度 13.0% ｜ 同源对最低 100% ⇒ **分离间隙 0.87**；类型判定端到端 **14/14 与真值一致**
- 🛑 **标定逼出的真缺陷（已修）**：`contentSimilarity` 旧实现**只累加共有块** ⇒
  **12 个「骨架块」**（`main`/`jailbreak`/`charDescription`…，真实库实测「出现在全部预设中」）
  正文天然完全相同 ⇒ **两个毫不相干的预设也算出内容相似度 100%** ⇒
  被误判「🔍 高度相似」而非「⚠️ 名称相同但无关」= **最危险的一类误判**。
  修：改 **Jaccard 语义**（分母含单侧独有块）⇒ 无关对 **100% → 6.1%**
- ⚠️ **诚实声明**：以上为**合成样本**标定，文本分布 ≠ 真实预设（真实库 118~261 块、正文可达 MB、中英混排）；
  只证明「算法判别力在已知差异类型上成立」，**不能**替代真实库标定
  （PK-29 的教训：合成样本定的 T=19 在真实库误报 10/18）。待真实库 20+ 预设 / 10+ 对同源后重跑标定脚本

### 📇 文件格式能力表（DF-22 根治，2026-09-24）

- **修复「导入能进、扫描不进」**：新增 `main/cardFormats.json`（**唯一权威格式表**）
  - `main/cardFormats.js`（CJS 封装）；`main.js` 两处扫描白名单改走 `isScannable(ext)`，
  `HeaderBar.vue` 的 `accept` 改为由格式表派生（渲染层经 JSON 导入，避免 Vite 转换风险）
- **🔑 关键取舍（选「移除 JPEG」而非「扩大白名单」）**：`file:saveCard` **只支持 `.json`/`.png`**
  （源码原话「webp 无法回写数据」）⇒ **JPEG/WebP 卡无法把标签/分类/编辑写回文件**；
  扩大白名单只会**把「静默不落盘」的陷阱扩大到更多格式** ⇒ 收敛到「可读且可写」的格式
- 守卫：`test/cardFormats.test.mjs`（**16 条**，含**源码级**断言三处入口同源）
- **连带发现 DF-25**（WebP/JPEG 卡**内容编辑不落盘**被**静默吞掉**，与 DF-20「头疼捂嘴」同型）——
  ⚠️ **现象核实修正**：初判写「改标签/分类后重启就没了」是**错的**；逐行核实确认
  **标签/分类不丢**（`cardOverlays` 覆盖层持久化到 `app_config.json`，重扫按 path 恢复），
  **丢的是内容编辑**（描述/人格/场景/开场白/内嵌世界书）。
  ✅ **真修复（保存时自动升级为 PNG）**：`file:saveCard` 新增 `.webp` 分支
  （转 PNG + 内嵌卡数据 + 写前校验 + 快照备份 + 原子写 + 删旧文件），返回 `newPath`；
  渲染层**同步 path 及所有按 path 派生的键**（覆盖层 / 导入时间 / 聊天键）。
  🔑 **为何「转 PNG」是对的**：**SillyTavern 自己就是这么做的** ——
  `character-card-parser.js` 把卡数据写进 PNG tEXt 块，`writeCharacterData()` 输出路径
  **硬编码 `.png`** ⇒ 酒馆保存任何卡都产出 PNG。用户**不需要自己转格式**。
  📌 **反面教训（走错的弯路）**：初版方案是「拒绝保存 + 提示用户自己用换卡图转格式」，
  用户当场指出「**我们无法强迫用户，只能适应适配**」——核查 ST 源码后发现
  **转格式本就是酒馆原生行为**，该方案是把项目的能力缺陷**转嫁给用户**。
  验证：`scripts/probes/_probe-webp-save-upgrade.mjs`（**端到端真实，非模拟**）
  用 sharp 真造 WebP 卡 → 走 main.js 的转换逻辑（**从源码提取函数体**）→ 读回断言，**5/5 全过**

### 📌 待条件（本次未做）

- 预设查重的**真实库标定**：需真实预设库补到 20+ 个 OpenAI 预设 / 10+ 对同源（脚本已就绪，届时只需重跑）

---

## 🩹 v2.3.0 · 世界书保存格式修复 + 分组与标签（2026-09-24）

> 缺陷登记：[DF-21](docs/bugs/BUG-数据与文件.md)（世界书保存丢失 ST 原生字典形态）/ [DF-26](docs/bugs/BUG-数据与文件.md)（分组哨兵碰撞）
> 规格：[`docs/规格与计划/世界书大库/世界书库分组与标签-实现规格.md`](docs/规格与计划/世界书大库/世界书库分组与标签-实现规格.md)（`RFC-20260921-WB-TAGS-02`）
> 新增模块：`js/utils/wbGroupsTags.js`（纯函数）｜ 新增单测：`wbGroupsTags.test.mjs`（15 条）
> 新增探针：`_probe-wb-uid-dict.mjs`（真实库端到端）

### ✨ 用户可感知的变化

- **世界书库支持标签了**：以前只能分组（一本书只能在一个组里），现在还能贴标签 ——
  可以跨分组找书（如「所有带『高武』的书」），也能按标签批量整理
- **分组可以整组改名 / 解散了**：以前改分组只能一本一本改；现在右键分组就能
  **整组重命名**，或**解散**（组内全部移回「默认」，**不会删除任何书**）
- **筛选更灵活**：分组与标签可以**组合筛选**（同时满足才显示）
- **修复：某些世界书保存后，格式与酒馆原生不一致** —— 已修复。
  以前保存后会把酒馆的「词条索引表」结构改成列表形式，虽然能读但已偏离原生格式；
  现在保存后**保持与酒馆完全一致的结构**（含词条编号的空缺，编号不会错位）

### 🧩 实现要点（内部）

- **DF-21 真修复**：`main/cardFieldSanitizer.js` 新增 `restoreEntriesDict()`
  （`wb:save` / `wb:create` 调用）—— 保留数字 uid（**含空洞**）、缺失/本应用临时串按 ST 语义
  （`getFreeWorldEntryUid`：**最小空闲整数**）分配、字典键与词条内 uid **双写**；
  ⚠️ **只对独立世界书**（内嵌 `character_book.entries` 是数组，绝不能转）。
  🛑 **含一次重要的「错误结论修正」**：前一天曾判定「uid = 数组下标」并推荐「按下标重写」——
  **该结论是错的**（样本恰好全是未删过词条的书）；实测 `炎孕-副本01.json` 有 397 条但
  uid 范围 **0~13331**（有空洞）；**ST 源码定论**：uid 是「词条身份」，删除会腾出号码供复用。
  ⇒ 教训：**先读上游源码定语义，再用数据验证**（光有形态分布、样本不具代表性仍会得出错误结论）
- **DF-26 修复 + A2 落地**：新增 `js/utils/wbGroupsTags.js`（哨兵常量 / 规范化 / **碰撞防护** /
  标签去重切换频次 / **复合过滤** `matchWbFilter`）；`useWorldbooks` 新增
  `getWbTags` / `setWbTags` / `toggleWbTagOn` / `addWbTagsBatch` / `wbAllTags` /
  `renameWbGroup` / `deleteWbGroup`；配置层新增 `wbTagMap`（与 `wbCategoryMap` 同源持久化）
- **📌 三个已拍板决策**：① 元数据**只留配置文件**（不动用户文件、不产快照、不碰第三方格式）
  ② **保持中文哨兵「全部」「默认」** + 补碰撞防护 ③ **角色卡随卡片走 / 独立世界书留系统配置**
- ⚠️ **原方案 ①②③ 项决策后全部不需要**（不写文件 ⇒ 无新 IPC、无快照、无第三方格式风险）；
  ④ 原方案**改错了模块**（世界书检索在 `useWorldbooks.filteredWorldbooks`，`useSearch.js` 只服务角色卡）
- ⚠️ **踩到的坑**（已修）：`getWbTags` 等新 API 只加进 ctx 却**没从 `useWorldbooks` 解构** ⇒
  启动冒烟报 `getWbTags is not defined`（AR-13 同型）——**冒烟测试抓到的**，编译期看不出来
- **验证**：`npm test` **681 用例全绿**（54 文件）；`npm run guard:batch-read` 通过；
  `npm run build:web` 通过；真实启动冒烟无 `[Vue 错误]`；
  `_probe-wb-uid-dict.mjs`（真实库 26 本 / 23,816 条）**26/26 变字典 + 26/26 自洽 + 21/21 uid 保真 + 26/26 条数不变**

### 📌 待条件（本次未做）

- 世界书**检索索引化**（≥1000 本）：规格列为待实测项（现状是单字段 `includes`，词条正文入索引会显著抬高体积）

---

## 🩹 v2.3.0 · AI 打标窗口布局重构（2026-09-22）

> 规格：[`docs/规格与计划/AI打标/AI打标窗口-UI重构-实现规格.md`](docs/规格与计划/AI打标/AI打标窗口-UI重构-实现规格.md)
> ｜ 后续升级（**搁置** → **已落地** 2026-09-24）：[`AI打标-提示词分角色与结构化输出方案.md`](docs/规格与计划/AI打标/AI打标-提示词分角色与结构化输出方案.md)
>
> - [`后续升级计划.md`](docs/规格与计划/计划与记录/后续升级计划.md) §五（「模型自由输出 → 系统选取」新链路）
...
- 新增铁律「**测试一律上真实库**」（读操作跑真实库 / 写操作用隔离库，配置仍隔离）——
  ...
  `docs/发布/规范与流程/一条龙-发布流程.md`、`docs/技术支持/README.md`
...
## 🩹 v2.3.0 · PK-27 架构改造：世界书「查重只读索引」+ 加载速度实测 + 进度条修复（2026-09-22）

> 方案：[`世界书大库-加载与查重架构方案.md`](docs/规格与计划/世界书大库/世界书大库-加载与查重架构方案.md)（v2.1，🟢 主体已落地）
> ｜ 实验：[`S0.5-simhash特征方案实验报告.md`](docs/规格与计划/世界书大库/S0.5-simhash特征方案实验报告.md)
> ｜ 计划：[`后续计划-20260922-PK27架构改造收尾.md`](docs/规格与计划/计划与记录/后续计划-20260922-PK27架构改造收尾.md)
> ｜ 数据：[`技术数据-大库压测与性能.md`](docs/技术支持/性能与稳定性/技术数据-大库压测与性能.md) §12.15
...
## 🩹 v2.3.0 · 查重 / 扫描 / 检索全链路修复（Phase 1 + Phase 2 + Phase 3，2026-09-21）

> 规格：[`docs/规格与计划/查重引擎/查重扫描与检索-最终方案.md`](docs/规格与计划/查重引擎/查重扫描与检索-最终方案.md)
> ｜ 流水：[`实施工作日志.md`](docs/规格与计划/查重引擎/查重扫描与检索-实施工作日志.md)
> ｜ 待办：[`剩余任务.md`](docs/规格与计划/查重引擎/查重扫描与检索-剩余任务.md)
> ｜ 移动版：[`移动版同步-待办清单.md`](docs/规格与计划/测卡工作区/移动版同步-待办清单.md)（独立成文）
...
## 🩹 v2.3.0 · 世界书 500 份压力测试专项（2026-09-22）

> 用户诉求：「复制 500 份做压力测试、改写某几个文件做对比测试、再测其他功能查开发中产生的意外 BUG」
> ｜ 数据：[`技术数据-大库压测与性能.md` §十二](docs/技术支持/性能与稳定性/技术数据-大库压测与性能.md)
...
## 🩹 v2.3.0 · PK-21 差异比对假阳性修复（2026-09-22）
...
## 🩹 v2.3.0 · PK-20 世界书大库扫描崩溃修复（2026-09-22）
...
## 🩹 v2.3.0 · 查重进度条 UX 三问修复（2026-09-22）

> 用户反馈：① 进度条数字为什么是「0 / ?」不动；② 第二次查重只看到进度条、结果却早已列出；
> ③ 扫描 / 对比弹窗有没有卡顿点
> ｜ 数据：[`技术数据-大库压测与性能.md` §12.8](docs/技术支持/性能与稳定性/技术数据-大库压测与性能.md)
...
## 🩹 v2.3.0 · 世界书库「⚡ 秒开」+ 懒加载真因修复（2026-09-22）

> 用户要求：「我要他的加载和角色卡一样进行**秒开**程序秒加载的效果」
> ｜ 数据：[`技术数据-大库压测与性能.md` §12.10](docs/技术支持/性能与稳定性/技术数据-大库压测与性能.md)
...
## 🩹 v2.3.0 · 世界书 5000 本极端压力测试（2026-09-22）

> 用户要求：「上压力测试上 5000 本书开始压力测试…同时测试其他改动的和未改动的功能来确保没有其他 BUG 诞生」
> ｜ 数据：[`技术数据-大库压测与性能.md` §12.11](docs/技术支持/性能与稳定性/技术数据-大库压测与性能.md)
...
## 🩹 v2.3.0 · 秒开回归第二处：世界书「列表态」字段缺失的消费者全面清理（PK-26，2026-09-22）
...
>
> - CT-19 卡内面板全局库缺失修复 + Python 一键检查工具。规格：`docs/规格与计划/功能规格/角色卡插件页签-实现规格.md`。
...
> 文档：`docs/技术支持/世界书与卡片/方案-卡片自动分组.md` **v2.1**（规格：`docs/规格与计划/功能规格/卡片自动分组-实现规格.md`）。
> 规格：`docs/规格与计划/AI打标/打标三层开关-P1实现规格.md`｜方案：`docs/技术支持/插件与扩展/方案-界面重整与应用级扩展系统.md`（v1.2）
...
- 文件：`docs/技术支持/世界书与卡片/方案-卡片自动分组.md`（**约 449 行**）
...
- 规格：`docs/规格与计划/AI打标/打标三层开关-P1实现规格.md`（S1~S4 分步实施与 3 条硬验收）
- 方案：`docs/技术支持/插件与扩展/方案-界面重整与应用级扩展系统.md`（P1 设计 + 评审结论 + §八 变更风控）
...
- 方案：`docs/技术支持/插件与扩展/方案-界面重整与应用级扩展系统.md`（P2 实施进度 + 偏离清单 + 新缺陷记录见 §十二；P2-2 表末“菜单入口补齐”项已按用户澄清修正口径）
...
- 存档位置：[`docs/规格与计划/计划与记录/后续升级计划.md`](docs/规格与计划/计划与记录/后续升级计划.md) 第四节（实测难度基线 + 重启前置清单）
...
- **规格**：`docs/规格与计划/功能规格/角色卡插件页签-实现规格.md`
...
- 新规格：`docs/规格与计划/功能规格/角色卡插件页签-实现规格.md`（含 76 张卡实测表、踩坑、CDP 验收记录）
...
>
> - CT-17/CT-18 逐条删除入口补齐。规格：`docs/规格与计划/测卡工作区/桌面版测卡记忆v4.1-实现规格.md`。
...
- **同号重发只能靠手动重装触达已装用户**；要让 OTA 覆盖全部用户，必须升版本号 ——
  这与 [`docs/发布/规范与流程/一条龙-发布流程.md`](docs/发布/规范与流程/一条龙-发布流程.md) §8「发布后回滚/修正要开新版本」一致。
...
> 汇总技术数据（加载分项、堆构成审计、缓存 A/B、容量边界）见 `docs/技术支持/性能与稳定性/技术数据-大库压测与性能.md`；
...
- 完整数据：`docs/技术支持/性能与稳定性/技术数据-大库压测与性能.md`（含 22k 库前后对比、缓存 A/B、内存审计口径）。
...
- 新增「卡内 HTML 渲染形态与已知缺口」总表：`docs/规格与计划/测卡工作区/桌面版测卡工作区-实现规格.md` §4.1/§4.2
...
- 接入方式：`📂 打开插件目录`（本地扫描）；原「Git 仓库链接导入」已剥离封存（详见 `docs/技术支持/世界书与卡片/代码片段-Git导入封存.md`）
