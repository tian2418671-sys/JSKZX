...
- **修复**（规划中，见 [`../规格与计划/查重扫描与检索-最终方案.md`](../规格与计划/查重扫描与检索-最终方案.md) §四 S2-5）：
...
- **验证**：单测 `test/searchPhrase.test.mjs` **9 条全绿**（含「索引就绪前后结果一致」「构建中/覆盖不足不漏卡」「索引查空回落」）；
  **UI 级端到端**（`scripts/_probe-search-phrase.mjs`，**8/8 通过**）：向真实应用注入「诱饵卡（体系 传统）」+「真卡（本系统用于监控）」，
...
- **修复**（规划中，优先级低于 PK-18 的准确性修复）：候选上限 / 前缀专用结构，方案待实测（见最终方案 §七 #1）。
- **来源**：2026-09-21 查重链路专项实测
