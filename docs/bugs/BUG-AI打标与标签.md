...
- **开关的真实契约**（`App.vue` 注释）：「开启后**仅保留自动分类结果**」—— 开关开启时导入只保留**分类**，不附加任何标签（既不含卡片自带，也不含规则生成）。
  > 📌 **此契约已于 v2.2.5 被取代，阅读时务必注意**（2026-09-13 回归审计标注）：
  > v2.2.5 把原先「一个开关既管清洗又管打标」拆成**双开关解耦**（见 AR-25）——
  > 本开关现在**只管**「是否清空卡片自带的原生 `data.tags`」，
  > 规则标签是否附加改由另一个开关 `autoTagOnImport` 独立控制。
  > 现行口径见 `App.vue:754-758` 注释与 `useCardCrud.js:287-310` 实现，
  > 并由 `test/sanitizeImport.test.mjs`「忽略开 + 规则开：清原生 + 规则贴标 + 自动分类」锁定。
  > **⚠️ 切勿照上面这行旧契约去"修复"** —— 那会把规则标签重新压回本开关，直接回归 **AR-25**。
...
## 六、LLM 层输出解析（AI-09、AI-10）

### AI-09 ｜ 🔴 贪婪正则导致「思考型模型」标签被思维链污染
- **现象**：用**思考型模型**（GLM / DeepSeek 等会先输出推理过程的模型）打标时，标签里**混进大量无关内容** —— 推理段落、编号、示例文字全被当成标签写入卡。
- **根因**：旧解析 `rawReply.match(/\[[\s\S]*\]/)` 是**贪婪匹配** —— 取「**第一个 `[`**」到「**最后一个 `]`**」。
  思考型模型的思维链里必然出现方括号（编号列表 `[1]`、示例 JSON `["示例", "格式"]`），
  贪婪匹配把整个「思维链 + 真结果」跨成一个区间 ⇒ 解析出的「标签」= 一堆解释文字。
- **修复**（`js/utils/llmPromptRoles.js`，**仅「只有 LLM 层启动」时启用**，见 §六）：
  1. **结构化输出契约**：要求模型把结果包在 `<tags>[...]</tags>`（兼容 `<标签>`），解析用**精确边界**；
  2. **从后往前找**：`<tags>` 包裹与 JSON 数组**均取「最后一个能解析成数组的候选」** ——
     ⚠️ 这是关键：思考型模型常在思维链里先写「示例：`<tags>["示例"]</tags>`」，**取第一个会把示例当结果**；
  3. **三层降级**（任一层成功即可）：`<tags>` 精确边界 → JSON 数组正则（从后往前 + 严格 `JSON.parse` 判定）→ 按标点暴力拆分；
     层② 额外加**贪婪兜底 + `flat(Infinity)`** 处理嵌套数组 `["a",["b"]]`（非贪婪会截断成非法 JSON）；
  4. 命中**第③层**时打 `⚠️` 日志告知用户「模型未遵守输出格式」（最脏的一层，不静默）。
- **加固（2026-09-24 第二批）**：
  5. **🧠 思维链（CoT）提示词**（三档 `cotMode`：默认版 / 自定义 / 关闭）—— 内置默认版融合
     「思维链引导 + 破限」，以 `assistant` 角色插在 `user` 之后（Anthropic 硬要求），
     并**明确要求「推理过程不要输出」**（从源头减少污染）；
  6. **🧹 解析前先剥思考块**（`stripThinkingBlocks`）：支持 `<thinking>` / ` thinking` / `<reasoning>` /
     `<analysis>` 与中文 `【思考】` / `【推理】` / `【分析】`；
     🛡️ **剥完若解析失败则回退原文**（防模型把真结果写在思考块里被误剥）。
- **防再犯**：
  - **凡解析 LLM 自由文本，禁止用贪婪 `[\s\S]*` 跨越区间**；优先「结构化包裹 + 精确边界」，且**取最后一个有效候选**；
  - 解析**必须**有多层降级 + 命中层可观测（否则「模型不听话」会表现为静默乱码）；
  - **解析前先剥已知的「模型自用标签」**（思考块 / 代码围栏），但**必须有回退**（剥过头会丢真结果）；
  - 写解析单测时**必须包含「思维链污染」用例**（链里有 `[1]`、`["示例"]`、示例包裹），不能只测干净输出。
- **来源**：AI 打标提示词分角色与结构化输出（2026-09-24）｜方案：`docs/规格与计划/AI打标/AI打标-提示词分角色与结构化输出方案.md` §九
- **验证**：`test/llmPromptRoles.test.mjs`（**70 条**，含思维链污染 / 多包裹取最后一个 / 嵌套数组 / 思考块剥离与回退 / 思维链三档模式用例）；`npm test` = 751 用例全绿

### AI-10 ｜ 🟡 预填充续写「解析缺口」——回复不含开头标记 → 全落第③层兜底（`</tags>` 混入标签并写盘）
- **现象**（2026-09-25 真实打标首测，gemini 假流式中转 · 仅 LLM 组合）：9 张卡**全部**命中第③层兜底拆分（日志每张都有 ⚠️），**最后一项“标签”是 `</tags>`**（如 `…、救赎/补偿、</tags>`）；另有中文引号残留（`“奇幻/异世界”`）、中文字间空格（`现代/都 市`）、斜杠旁空格（`温柔/ 体贴`）。脏项经 `applyAutoTags` 双写（覆盖层 + PNG `data.tags`）落盘。
- **根因（两层）**：
  1. **结构根因（主因）**：发送侧放入 assistant 预填充（`<tags>[`，意图「模型接着从这里写」），API 返回的回复**只含续写部分**（预填充文本不在回复里）——而解析侧要求回复**自含**完整 `<tags>…</tags>` 或 JSON 数组 → 层①②**结构性失配**：任何「正常续写」都够不着①/② → 全部被推给③「无脑拆分」；模型补的闭合标记 `</tags>` 正好落在「内容区」→ 被当标签。**「每张卡都走③」在这种形态下是必然，不是个例**。
  2. **清理缺位（次因）**：层③ 只清 ASCII 方括号/引号 —— 不剥 `<tags>/</tags>` 标记、不清中文引号、不折叠中文字间与斜杠旁空格 ⇒ 边界标记 / 空白变形直接落盘。
  > 📌 日志文案「模型未遵守输出格式」在预填充形态下**名不副实** —— 修法不是继续补③的清理（那只是捂嘴），而是补上「回复=续写」的一致性，让③回归「真乱写才用」的最后防线。
- **修复（根治 + 纵深）**：
  1. **根治（结构一致性）**：
     · `parseStructuredTags(rawReply, { prefill })`：组装侧把**本次实际使用的预填充**（`requestTagging` 新增返回 `usedPrefill`，随降级阶梯变化）传给解析侧；解析先「**拼回完整结构**」（`prefill + 回复`）再走三层 → ①②按设计正常命中；原文已含完整包裹时不拼接（既有行为不变）、拼接失败仍回退原文；
     · **打包请求禁用预填充**（`disablePrefill`）：对象格式 `<tags>{…}</tags>` 与数组预填充 `<tags>[` 本就冲突——禁掉后两端不再打架（此前实测「打包 2 张全失败 → 拆单」即此冲突）。
  2. **纵深（最脏输入防线）**：新增 `sanitizeTagList()`（**按最脏输入设计，一次做全**）：
     · 字符级——空白变形归一（NBSP / 全角空格 / 零宽 / Tab）→ 剥 `<tags>/<标签>` 包装标记 → 剥引号全谱（`" ' \` “ ” ‘ ’ 「 」 『 』`）与方括号 → 剥 Markdown 粗斜体（`**` / `__`）→ 剥行首列表符 / 编号（`- ` / `• ` / `1. ` / `2、`）→ 剥首尾残留标点（`、，,；;：:。`，ASCII `.` 不剥防误伤 `v1.2`）→ 折叠斜杠两侧与中文字间空格；
     · 项级门槛——空项 / 纯标记 / 纯数字 / 超长（> 40 字符，疑似解释文字）→ 丢弃；去重保序；
     · 接入点：层①-b（拼接后成为常规路径，包裹内纯文本同样过清洗）+ 层③ + `useAITools.js` 两处非 llm-only 兜底拆分。
- **验证**：
  - `test/llmPromptRoles.test.mjs` 新增 **19 条**用例（预填充拼接 5 条 + 兜底清理 14 条）；`npm test` **761 全绿**；
  - 副本库端到端（续写形态 mock `热测标签A、热测标签B、</tags>`）**10/10**：单卡链标签干净落盘且日志**不含**「走第③层兜底拆分」（根治生效）；打包请求 `roles=[system,user]`（**无 assistant 段**，预填充已禁）；
  - 真实小库 9 张已污染卡清洗后**全频谱复核幂等通过**（走 `persistCardUpdate` 正常落盘路径）。
- **防再犯**：
  - **发送形态与解析假设必须对齐**：「assistant 预填充 = 回复是续写」——凡新增/改动预填充，必须同步检查解析侧是否拼回（`usedPrefill` 透传）；
  - **兜底层 = 最脏输入的现场**：清理按“最脏”设计（标记 / 引号中英 / 空白变形 / 纯噪声项），且**解析成功 ≠ 内容干净**；
  - **格式契约冲突要先排查**：如「打包对象 `{…}` vs 预填充 `[`」——同类冲突在新增输出格式时必须先对一遍。
- **来源**：v2.2.14 第二批打标改造 · 真实小库打标首测 + 根治（2026-09-25）
...
5. 改**LLM 输出解析**：按 [AI-09](BUG-AI打标与标签.md#ai-09--贪婪正则导致思考型模型标签被思维链污染) —— 禁止贪婪跨区间、必须多层降级、单测必须含**思维链污染**用例。
   **另（AI-10）**：凡涉及**预填充 / 输出格式契约**，发送侧与解析侧必须对齐（`usedPrefill` 透传 + 拼回解析）；新增输出格式先排查与预填充的冲突（如对象 `{…}` vs `[`）。
6. 改**提示词预设字段**：旧预设只有 `content`，新增字段后**必须保持 `content` 镜像**（`normalizePromptPreset` 迁移 + UI 双写），否则降级回旧版本会丢数据。
7. 改**消息组装（新增 assistant 段）**：⚠️ **所有 `assistant` 消息必须排在 `user` 之后** ——
   Anthropic 要求第一条非 system 消息必须是 `user`，放前面会直接报错（`main.js` 挑出 `system` 后原样透传）。
8. 改**末尾 assistant 段（预填充 / 思维链）**：必须**同步降级阶梯**（`useAITools.js` 的 `ladder`），
   否则部分中转站拒收时整卡打标失败。
