> 📎 位置说明（2026-09-13 文档整理）：本文件原为 `docs/history/DM-世界书条目名修复与角色卡导入.md`，现归入**技术支持**文档。
> 其中「BUG 反馈：世界书导出条目名缺失」的缺陷记录已归并到 `docs/bugs/BUG-数据与文件.md`（**DF-07**）—— **缺陷与根因看那里，修复代码看这里**。
> 本文档为修改方案文档，代码块给出精确的文件、定位锚点与替换内容，可照抄落地。
