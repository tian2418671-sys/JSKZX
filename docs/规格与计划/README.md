...
| [AI打标-提示词分角色与结构化输出方案.md](AI打标-提示词分角色与结构化输出方案.md) | ⏸️ **搜置转后续**（2026-09-22）：提示词分 `system`/`assistant`/`user` + `<标签>` 结构化输出截取 + **「模型自由输出 → 系统选取」新链路**——含现状取证、技术洞察（思考型模型 CoT 污染贪婪正则）、3 个候选方案对比（**推荐 2️⃣**：用「留空即默认」替代模式开关）、二步走路线与 Q1~Q3 + S1~S6 待拍板；重启入口 [`后续升级计划.md`](后续升级计划.md) §五 | ⏸️ 已搁置转后续 |
...
| [世界书查重-多维度方案评估.md](世界书查重-多维度方案评估.md) | **外部「多级漏斗查重方案」的逐项评估 + 第 1 步落地记录**（2026-09-23）：✅ **采纳** 多维度加权 / 长度惩罚 / 相似类型分类（6 类型：完全重复·触发重复·内容可合并·设定冲突·高度相似·仅名称相同）/ 酒馆特化判定规则（**数据全在手，零额外成本**）；❌ **否决** 句级对齐（实测只能改善 **1 条词条 = 0.02%**，因行内 token 级 diff 已覆盖 99.98%）、7 个 Python/R 工具链（违反「零原生依赖」）、FAISS/ColBERT（量级错配：34.8GB ≈ 24.6TB token 嵌入）、8 维权重（仅 8 对真样本 → 过拟合）；🚫 **危险** 「连通分量」聚类（**正是 PK-29 刚修掉的病根**）。含六类型判据表、落地文件清单、以及「类型判定**绝不参与分组**」的契约断言 | ✅ 第 1 步已落地（583 用例全绿）；第 2~3 步待做（**前置：标注集扩到 50+ 对**） |
