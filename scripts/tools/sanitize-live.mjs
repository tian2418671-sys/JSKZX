 ...
 * 用法（顺序）：
 *   node scripts/tools/sanitize-live.mjs probe                 # 探测状态 / __vue_app__ 可达性
 *   node scripts/tools/sanitize-live.mjs set <true|false>      # 置开关（场景切换）
 *   node scripts/tools/sanitize-live.mjs import <json...>      # 注入文件 input 触发真实导入并轮询出现
 *   node scripts/tools/sanitize-live.mjs check <name> [name...] # 读回卡片状态 + 全局池是否含外来标签
