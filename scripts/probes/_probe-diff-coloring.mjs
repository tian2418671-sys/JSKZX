 ...
 *
 * 用法（dev 模式 + CDP）：
 *   $env:CDP_PORT="9366"; node scripts/probes/_probe-diff-coloring.mjs
 *
...
const L_LAST = '最后一行。';
const L_ONLY_NEW = '这一行只存在于新版。';  // ★ 供 emerald（对端新增）用例
...
const ITEM_B = {
    ...
    data: {
        name: '测试世界书', entries: [
            ...
            mk(0, ['主角', 'protagonist', '主角'], '主角设定', [L1, L2B, L3, L_ONLY_NEW].join('\n')),  // ★ 多一行 → 产生 added（emerald）
            // 同一词条：删掉中间一行
...
(async () => {
    ...
    const opened = await evaluate(`(async () => {
        ...
        // ⚠️ Vue 3.5 变更（历史记录 AR-XX）：app._instance **恒为 null** ——
        //    正确路径是 app._container._vnode.component.provides.appCtx（旧探针用 _instance 已失效）
        const inst = (app._container && app._container._vnode && app._container._vnode.component)
            || app._instance || null;
        const ctx = inst && inst.provides && inst.provides.appCtx;
