 ...
 *
 * 用法：
 *   node scripts/png-head-io-probe.mjs                        # 默认 I:\03\角色色卡
 *   node scripts/png-head-io-probe.mjs --library "D:\某库"
 *   node scripts/png-head-io-probe.mjs --dry                # 只统计不实读
 *   node scripts/png-head-io-probe.mjs --concurrency 128     # 默认与应用一致
 */
...
(async () => {
    ...
    console.log(`\n💡 实测冷启动：重启电脑后第一时间执行  node scripts/measure-startup.mjs --label 冷启动 --timeout 240000`);
