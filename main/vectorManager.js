...
const batchMatch = async (cards, labelPool, topK = DEFAULT_TOP_K, threshold = DEFAULT_THRESHOLD, modelName = DEFAULT_MODEL, onProgress) => {
    ...
    // 1. 加载或构建标签向量索引（持久化缓存，重启不重算）
    let labelVectors = await loadIndex(labelPool, modelName);
    if (!labelVectors) {
        const rawBuffers = await embed(labelPool, EMBED_BATCH);
        labelVectors = rawBuffers.map((buf) => new Float32Array(buf));
        await saveIndex(labelPool, modelName, labelVectors);
    } else {
